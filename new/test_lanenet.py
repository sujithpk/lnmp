...
def test_lanenet(image_path, weights_path, use_gpu):
    ...
    log.info('Reading and pre-processing image data')
    t_start = time.time()
    image = cv2.imread(image_path, cv2.IMREAD_COLOR)
    image_vis = image
    image = cv2.resize(image, (512, 256), interpolation=cv2.INTER_LINEAR)
    image = image - VGG_MEAN
    log.info('Preprocessing completed. Time taken: {:.5f}s'.format(time.time() - t_start))
    ...
    with sess.as_default():
        ...
        log.info('Lane detection complete.. Time taken: {:.5f}s'.format(t_cost))
